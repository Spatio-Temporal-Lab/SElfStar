package org.urbcomp.startdb.selfstar.mixcompressor;

import javafx.util.Pair;
import org.urbcomp.startdb.selfstar.compressor.ICompressor;
import org.urbcomp.startdb.selfstar.mixcompressor.xor.IXORCompressor;
...
public class ElfStarMixCompressor implements ICompressor {
    ...
    private final IXORCompressor[] xorCompressors;
    ...
    private IXORCompressor xorCompressor;

    public ElfStarMixCompressor(IXORCompressor[] xorCompressors, int window) {
        this.xorCompressors = xorCompressors;
        this.capacity = window;
        this.os = new OutputBitStream(
    ...
    public ElfStarMixCompressor(IXORCompressor[] xorCompressors) {
        this.xorCompressors = xorCompressors;
        this.os = new OutputBitStream(
                new byte[(int) (((capacity + 1) * 8 + capacity / 8 + 1) * 1.2)]);
    ...
    private void initXORCompressor() {
        boolean a = true;
        if (true) {
            xorCompressor = xorCompressors[0];
        } else xorCompressor = xorCompressors[1];
        xorCompressor.setOutputBitStream(this.os);
    }
