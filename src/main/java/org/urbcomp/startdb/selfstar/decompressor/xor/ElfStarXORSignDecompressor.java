...
public class ElfStarXORSignDecompressor implements IXORDecompressor {
    ...
    public ElfStarXORSignDecompressor() {
    }
    ...
    public Double readValue() {
        ...
        return Double.longBitsToDouble(storedVal | sign);
    }
    ...
    private void next() throws IOException {
        if (first) {
            ...
            first = false;
            sign = in.readLong(1) << 63;
            int trailingZeros = in.readInt(7);
            if (trailingZeros < 64) {
                storedVal = ((in.readLong(62 - trailingZeros) << 1) + 1) << trailingZeros;
            } else {
                storedVal = 0;
            }
