...
public class ElfStarXORCompressorWOF implements IXORCompressor {
    ...
    private int initLeadingRoundAndRepresentation(int[] distribution) {
        int[] positions = PostOfficeSolverWOF.initRoundAndRepresentation(distribution, leadingRepresentation, leadingRound);
        leadingBitsPerValue = PostOfficeSolverWOF.positionLength2Bits[positions.length];
        return PostOfficeSolverWOF.writePositions(positions, out);
    }

    private int initTrailingRoundAndRepresentation(int[] distribution) {
        int[] positions = PostOfficeSolverWOF.initRoundAndRepresentation(distribution, trailingRepresentation, trailingRound);
        trailingBitsPerValue = PostOfficeSolverWOF.positionLength2Bits[positions.length];
        return PostOfficeSolverWOF.writePositions(positions, out);
    }
