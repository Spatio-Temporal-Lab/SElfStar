...
import org.urbcomp.startdb.selfstar.utils.PostOfficeSolverNoFRZPruning;

import java.util.Arrays;

// without first-rear pruning and zero-pruning
public class ElfStarXORCompressorNoFRZPruning implements IXORCompressor {
    ...
    public ElfStarXORCompressorNoFRZPruning() {
        int capacity = 1000;
    ...
    private int initLeadingRoundAndRepresentation(int[] distribution) {
        int[] positions = PostOfficeSolverNoFRZPruning.initRoundAndRepresentation(distribution, leadingRepresentation, leadingRound);
        leadingBitsPerValue = PostOfficeSolverNoFRZPruning.positionLength2Bits[positions.length];
        return PostOfficeSolverNoFRZPruning.writePositions(positions, out);
    }

    private int initTrailingRoundAndRepresentation(int[] distribution) {
        int[] positions = PostOfficeSolverNoFRZPruning.initRoundAndRepresentation(distribution, trailingRepresentation, trailingRound);
        trailingBitsPerValue = PostOfficeSolverNoFRZPruning.positionLength2Bits[positions.length];
        return PostOfficeSolverNoFRZPruning.writePositions(positions, out);
    }
