...
public class Elf32Utils {
    ...
    public static int getSignificantCount(float v, int sp) {
        int i;
        if (sp >= 0) {
            i = 1;
        } else {
    ...
    private static int getSignificantCount(float v, int sp, int lastBetaStar) {
        ...
        if (lastBetaStar != Integer.MAX_VALUE && lastBetaStar != 0) {
            ...
            i = 8 - sp;    // loop backwards. For datasets with big beta, we may not store the beta for the first several values
        } else if (sp >= 0) {
            i = 1;
        } else {
            i = -sp;
